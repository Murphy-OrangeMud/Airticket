...
if __name__ == '__main__':
    f = open("Info.txt", "w")
    country1 = input("Start Country: ")
    country2 = input("Destination Country: ")
    outset_date = input("Date (format: 2019-01-01): ")
    Destination = []
    cityInfo = CityInfo.CityCodeCrawler()
    Start = cityInfo.getDomesticCities()
    if country2 != '中国':
        Cities = cityInfo.getInternationalCities()
        for city in Cities:
            if city["country"] == country2:
                Destination.append(city["cityname"])
    else:
        city = input("Destination City: ")
        Destination.append(city)
    qf = QunarFlight()
    for city in Start:
        send_city_name1 = 'u' + city["cityname"]
        for city in Destination:
            send_city_name2 = 'u' + city
            datas = qf.find_event(send_city_name1, send_city_name2, date1=outset_date)
            # print(json.dumps(datas))
            f.write("from "+ send_city_name1 + "to" +send_city_name2 + "\n\r")
            f.write("set out time: " + json.dumps(datas)["set_out_date"] + "\n\r")
            f.write("arrival time: " + json.dumps(datas)["arrival_date"] + "\n\r")
            f.write("price: " + json.dumps(datas)["price"] +"\n\r")
            print("from "+ send_city_name1 + "to" +send_city_name2)
            print("set out time: " + json.dumps(datas)["set_out_date"])
            print("arrival time: " + json.dumps(datas)["arrival_date"])
            print("price: " + json.dumps(datas)["price"])
